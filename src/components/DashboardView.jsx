...
export const DashboardView = () => {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-10 max-w-7xl mx-auto space-y-8 md:space-y-10 relative z-10">
      ...
      {/* ── 1. Glassmorphic Hero Banner (Shows moving background through it) ── */}
      <div className="relative rounded-3xl p-6 sm:p-8 md:p-10 overflow-hidden border border-white/10 bg-slate-950/40 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.37),inset_0_1px_0_0_rgba(255,255,255,0.1)] group">
        
        {/* Floating internal plasma light */}
        <motion.div
          animate={{
            scale: [1, 1.25, 1],
          ...
          className="absolute -top-20 -right-20 w-96 h-96 rounded-full bg-gradient-to-br from-blue-500/30 to-cyan-400/20 blur-3xl pointer-events-none"
        />
      ...
      {/* ── 2. Alive Telemetry Metrics ── */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-slate-950/40 backdrop-blur-xl border border-white/10 rounded-2xl p-4 flex items-center justify-between shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05)] hover:border-cyan-500/30 transition-all">
          ...
          {semesters.map((sem) => (
            <GlassCard key={sem.id}>
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="w-12 h-12 rounded-xl bg-slate-900/90 border border-white/10 flex items-center justify-center text-xl shadow-inner group-hover:scale-105 transition-transform">
                  {sem.icon}
                </div>
                <span className={`text-[10px] font-bold tracking-wider px-2 py-0.5 rounded-md border ${
                  sem.tag === 'Active' ? 'bg-cyan-500/10 text-cyan-300 border-cyan-500/30 shadow-[0_0_10px_rgba(6,182,212,0.2)]' :
                  sem.tag === 'Completed' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                  'bg-slate-800/80 text-slate-400 border-white/10'
                }`}>{sem.tag}</span>
              </div>
              
              <h3 className="text-lg font-bold text-white mb-1 group-hover:text-cyan-300 transition-colors">{sem.title}</h3>
              <p className="text-sm text-slate-400 font-medium mb-4">{sem.subjects}</p>
              
              <div style={{ background: 'linear-gradient(to right, transparent, rgba(255,255,255,0.08), transparent)' }} className="w-full h-[1px] my-4" />
              
              <Link 
                to={`/semester/${sem.id}`} 
                className="w-full py-2.5 bg-slate-900/80 hover:bg-cyan-600 text-slate-300 hover:text-slate-950 hover:font-bold rounded-xl text-xs font-semibold tracking-wide flex items-center justify-center gap-1.5 transition-all border border-white/10 hover:border-transparent group/btn shadow-sm"
              >
